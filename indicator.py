...
def MAAMT(ohlcv: pd.DataFrame, n=40):
    ...
    maamt = bt.talib.MA(numpy.array(volume).astype(float), timeperiod=n)
...
def DPO(close_data: pd.DataFrame, n=20):
    ...
    before_day_end = int(n + before_day)
    close_arr = close_data[-before_day_end:-before_day]['close'].tolist()
    ref_data = bt.talib.MA(numpy.array(close_arr), timeperiod=n)  #
    close = close_data.tail(1)['close'].tolist()
    dpo = close[0] - ref_data[-1]
    return dpo


def ER(ohlcv: pd.DataFrame, n=20):
    close = ohlcv['close'].tolist()
    close_ema =bt.talib.EMA(numpy.array(close), timeperiod=n)
...
def tii_line(etf_data: pd.DataFrame, n1=40):
    ...
    close_ma = bt.talib.MA(numpy.array(close), n1)
    dev_nd_array = numpy.subtract(close[-m:], close_ma[-m:])
    for dev in dev_nd_array:
        dev_pos = dev if dev > 0 else 0
        dev_neg = -dev if dev < 0 else 0
...
def calc_tii_signal(etf_data: pd.DataFrame, n1=40, n2=9):
    ...
    tii_signal = bt.talib.EMA(numpy.array(tii_array), n2)
    return tii_signal[-1]
